...
public class TakeOutProductPagerAdapter extends RecyclerView.Adapter<TakeOutProductPagerAdapter.ProductPageViewHolder> {
    private List<Product> productList;

    public TakeOutProductPagerAdapter(List<Product> productList) {
    ...
    public int getItemCount() {
        return (int) Math.ceil((double) productList.size() / 7); // 每页4个商品
    }
    ...
    static class ProductPageViewHolder extends RecyclerView.ViewHolder {
        ...
        private TakeOutProductPagerAdapter adapter; // 添加适配器引用
        ...
        public ProductPageViewHolder(@NonNull View itemView, TakeOutProductPagerAdapter adapter) {
            super(itemView);
        ...
        private void selectRandomProduct(Context context) {
            if (adapter.productList.isEmpty()) {
            ...
            // 随机选择一个产品
            int randomIndex = (int) (Math.random() * adapter.productList.size());
            Product selectedProduct = adapter.productList.get(randomIndex);
